...
const paths = [
  ...
  'src/routes/admissions.lazy.tsx',
...
for (const p of paths) {
  ...
  content = content.replace(/style=\{\{ backgroundImage: "url\('https:\/\/lh3\.googleusercontent\.com[^']+'\)" \}\}/g, (match) => {
    const currentImg = images[imgIndex % images.length];
    const imgName = `bgImg${imgIndex}`;
    imports += `import ${imgName} from '../assets/images/${currentImg}';\n`;
    imgIndex++;
    return `style={{ backgroundImage: \`url(\${${imgName}})\` }}`;
  });
